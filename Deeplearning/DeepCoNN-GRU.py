...
# set of users and electronics so that our network never learns those
test_size = 0.005
...
unique_test_electronics = test["asin"].unique()

# drop the electronics that also appear in our test set. In order to be
# a true train/test split, we are forced to discard some data entirely
train = train.where(np.logical_not(train["asin"].isin(unique_test_electronics))).dropna()

user_seq_sizes = raw_data.loc[:, "userReviews"].apply(lambda x: str(x).split()).apply(len)
item_seq_sizes = raw_data.loc[:, "electronicReviews"].apply(lambda x: str(x).split()).apply(len)
...
class DeepCoNN():
    ...
    def train(self, train_data, batch_size, epochs=3500):
        ...
        electronic_reviews = np.array(list(train_data.loc[:, "electronicReviews"]))

        self.train_inputs = [user_reviews, electronic_reviews]
...
deepconn.model.save("gru.h5")

user_reviews = np.array(list(test_embedded.loc[:, "userReviews"]))
electronic_reviews = np.array(list(test_embedded.loc[:, "electronicReviews"]))

test_inputs = [user_reviews, electronic_reviews]

dat = pd.DataFrame(test_inputs)
dat.to_csv("test_data.csv")
